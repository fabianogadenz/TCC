...
\titulo{{ resgate de caracteres em embalagens de medicamentos com técnicas de processamento de imagem e OCR neuronal}} % titulo do trabalho em portugues
...
\cita{GADENZ, Fabiano} % sobrenome (maiusculas), nome do autor do trabalho

\palavraschave{Reconhecimento de caracteres, processamento de imagem, dispositivos móveis, medicamentos} % palavras-chave do trabalho
\keywords{Character recognition, image processing, mobile devices, medicines} % palavras-chave do trabalho em ingles
...
\orientador{Prof. Dr. Everton Coimbra de Araújo} % nome do orientador do trabalho
...
%resumo
\begin{resumo}
Devido ao crescente interesse das pessoas com mais de 60 anos pela tecnologia, é possível que os indivíduos estejam mais integrados a um ambiente de troca de informações por meio de seus telefones celulares. E considerando que o uso de múltiplos medicamentos é uma condição frequente em idosos, que, apesar de necessária, na maioria das vezes provém riscos em relação aos efeitos adversos na interação medicamentosa quando mal enjerida. No presente trabalho foi proposto uma solução para auxiliar o paciente em seu período de tratamento por meio de uma foto do medicamento que será identificada com o auxílio de técnicas de processamento de imagem e reconhecimento de caracteres, sendo possível informar o princípio ativo do medicamento de interesse.
\end{resumo}

%abstract
\begin{abstract}
Due to the growing interest of people over 60 in technology, it is possible for individuals to be more integrated into an information exchange environment through their mobile phones. And considering that the use of multiple medications is a frequent condition in the elderly, which, although necessary, most often predisposes to risks in relation to adverse effects and drug interaction. In the present work a solution was proposed to assist the patient in his treatment period by means of a photo of the drug that will be identified with the aid of image processing techniques and character recognition, being possible to inform the active principle of the drug of interest.
\end{abstract}
